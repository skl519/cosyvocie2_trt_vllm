...
class CosyVoice2Model(CosyVoiceModel):
    ...
    def __init__(self,
        ...
        self.encoder_cache = self.init_encoder_cache()
        self.decoder_cache = self.init_decoder_cache()
        if self.fp16 is True:
            for cache in [self.encoder_cache, self.decoder_cache]:
                for k, v in cache.items():
                    if isinstance(v, torch.Tensor):
                        cache[k] = v.half()

    def init_encoder_cache(self):
        return {
            'offset': 0,
    ...
    def init_flow_cache(self):
        encoder_cache = self.encoder_cache.copy()
        decoder_cache = self.decoder_cache.copy()
    ...
    def tts(self, text=torch.zeros(1, 0, dtype=torch.int32), flow_embedding=torch.zeros(0, 192), llm_embedding=torch.zeros(0, 192),
        ...
        if stream is True:
            ...
            while True:
                time.sleep(0.1)     # 
                if len(self.tts_speech_token_dict[this_uuid]) >= self.token_hop_len + self.flow.pre_lookahead_len:      # llm输出的speech_token >= 25+3
