...
class Qwen2LM(TransformerLM):
    ...
    def inference(
        ...
        for i in range(max_len):
            ...
            y_pred, cache = self.llm.forward_one_step(lm_input,masks=torch.tril(torch.ones((1, lm_input.shape[1], lm_input.shape[1]), device=lm_input.device)).to(torch.bool),cache=cache)
            #print(f'llm forward_one_step处理时间{time.time()-t1}')# 0.018718242645263672-0.010999917984008789
            ...
            #print(f'llm logp处理时间{time.time()-t2}')
            # in stream mode, yield token one by one
            yield top_ids
            #print(out_tokens)
